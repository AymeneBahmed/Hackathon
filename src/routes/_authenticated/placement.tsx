  ...
  return (
    <main className="mx-auto max-w-2xl px-6 py-12">
      ...
      <div className="rounded-3xl border border-border bg-card p-8 shadow-[var(--shadow-soft)]">
        <div className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">{q.skill} · {q.level}</div>
        ...
        <div className="mt-6 flex items-center justify-between">
          <button onClick={() => setI(Math.max(0, i - 1))} disabled={i === 0} className="text-sm text-muted-foreground hover:text-foreground disabled:opacity-30">← Previous</button>
          {i < total - 1 ? (
            <button onClick={() => setI(i + 1)} className="rounded-full bg-muted px-5 py-2 text-sm font-medium">Skip</button>
          ) : (
            <button onClick={finalize} disabled={!canFinish || busy} className="rounded-full bg-primary px-6 py-2.5 text-sm font-semibold text-primary-foreground disabled:opacity-50">
...
function Tile({ label, items, tone }: { label: string; items: string[]; tone: "success" | "warning" }) {
  const bg = tone === "success" ? "bg-accent/15 text-accent-foreground" : "bg-warning/15";
  return (
    <div className={`rounded-2xl p-4 ${bg}`}>
      <div className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">{label}</div>
      <div className="mt-2 text-sm font-medium capitalize">{items.length ? items.join(", ") : "—"}</div>
    </div>
  );
}
